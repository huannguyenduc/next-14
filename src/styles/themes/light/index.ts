...
import type { ChartColors, ColorType, ITheme, IndexedPrimaries } from '@/interfaces';
import { hexToRGB, shadeColor } from '@/utils';
...
const rgb = Object.fromEntries(
  Object.entries({
    ...
    ...chartColors,
    background,
  } satisfies ITheme['rgb']).map(([key, hexColor]) => [key, hexToRGB(hexColor)])
) as ITheme['rgb'];
